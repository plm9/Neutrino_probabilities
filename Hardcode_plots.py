...
#Here we do the same but for anti-neutrino
P_anti_ne_nmu=(np.sin(theta_23)**2)*(np.sin(2*theta_13)**2)*S_23+(np.cos(theta_23)**2)*(np.sin(2*theta_12)**2)*S_12+P_cp
...
for i,l in enumerate(dist):
    ...
    eq_a_em=P_anti_ne_nmu.subs([(L,l)])
    for j,en in enumerate(eneg_range):
        ...
        prob_anti_ne_nmu[i][j]=eq_a_em.subs([(E_nu,en)])
    del eq_em,eq_a_em,eq_ee,eq_mm,
...
plt.xlim(right=int(n*0.89))
#plt.title(r"$P(\nu_{e}\rightarrow\nu_{\mu})$",fontsize=14) 
plt.savefig("heatmaps/heatmaps_from_paper/HM_prob_e_mu_3pi2.pdf")
plt.show()
...
plt.figure(figsize=(8,6))
sns.heatmap(prob_anti_ne_nmu,cbar=False)
sns.heatmap(prob_anti_ne_nmu).figure.axes[-1].set_ylabel(r"$P(\bar{\nu}_{e}\rightarrow\bar{\nu}_{\mu})$",size=14) 
ymin,ymax=plt.gca().get_ylim()
xmin,xmax=plt.gca().get_xlim()  
...
plt.xlim(right=int(n*0.89))
#plt.title(r"$P(\nu_{e}\rightarrow\nu_{\mu})$",fontsize=14) 
plt.savefig("heatmaps/heatmaps_from_paper/HM_prob_anti_e_mu_3pi2.pdf")
plt.show()


plt.figure(figsize=(8,6))
sns.heatmap(prob_ne_nmu-prob_anti_ne_nmu,cbar=False)
sns.heatmap(prob_ne_nmu-prob_anti_ne_nmu).figure.axes[-1].set_ylabel(r"$P(\nu_{e}\rightarrow\nu_{\mu})-P(\bar{\nu}_{e}\rightarrow\bar{\nu}_{\mu})$",size=14) 
ymin,ymax=plt.gca().get_ylim()
xmin,xmax=plt.gca().get_xlim()  
...
plt.xlim(right=int(n*0.89))
plt.title(r"CP-violation plot for $\delta_{CP}=3\pi/2$",fontsize=14) 
plt.savefig("heatmaps/heatmaps_from_paper/HM_CP_diff_3pi2.pdf")
plt.show()
